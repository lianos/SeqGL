...
run.seqGL.wrapper <- function (pos.regions, neg.regions, org='hg19', res.dir,
  ...
  known.motifs=NULL, ...) {
	...
	if (motifs) {
		gm.res <- group.motifs(res.dir, dictionary.file,
                           no.cores=no.cores, org=org, test.classes=1,
                           known.motifs=known.motifs)
    saveRDS(gm.res, sprintf('%s/group_motif_res.rds', res.dir))
	}
}
...
##' Run SeqGL with prespecified pos and neg labels
##' @export
run_seqGL <- function (pos, neg, out.dir, data.type, org,
	span=150, max.examples=ifelse (data.type == 'ChIP', 5e3, 4e4),  no.cores = 1,
	no.groups=ifelse (data.type == 'ChIP', 20, 100),
	dictionary.file=system.file( "extdata/wildcard_dict_kmer8_mismatches2_alpha5_consecutive_mis.Rdata", package="SeqGL"),
  ) {
  ...
  feature.count <- 25000
  fdr.cutoff <- 0.01
  max.examples <- max(length(pos), length(neg))
  ...
  ## me
  all.regions <- pos
  pos.seqs <- SeqGL:::get.seqs(SeqGL:::load.bsgenome(org), pos)
  neg.seqs <- SeqGL:::get.seqs(SeqGL:::load.bsgenome(org), pos)
  ##
  ...
  neg.regions <- neg
	...
	# Remove overlaps
  opos <- pos.regions
  pos.o <- countOverlaps(pos.regions, neg.regions)
  pos.regions <- pos.regions[pos.o == 0]

  neg.o <- countOverlaps(neg.regions, opos)
  neg.regions <- neg.regions[neg.o == 0]

  N <- min(length(pos.regions), length(neg.regions))
  pos.regions <- head(pos.regions, N)
  neg.regions <- head(neg.regions, N)
	time.end <- SeqGL:::get.time()
	...
	# Run group lasso
  mcols(pos.regions) <- NULL
  mcols(neg.regions) <- NULL
	show ('Running SeqGL...')
	run.seqGL.wrapper (pos.regions, neg.regions, org, out.dir,
		dictionary.file, no.groups, no.cores = no.cores, fdr.cutoff=fdr.cutoff,
		motifs=TRUE, test.classes=1, feature.count=feature.count)
  ...
  seqs <- c(pos.seqs, neg.seqs)
	...
	all.features <- build.features.kernels(dictionary.file, seqs,
                                        kmers=colnames(train.test.data$train.features),
                                        verbose=FALSE)$features
	## all.labels <- rep (c(1, -1), each=length(all.regions))
  all.labels <- c(rep(1, length(pos.seqs)), rep(-1, length(neg.seqs)))

	# Group members setups etc
  stopifnot(all(colnames(all.features) == names(group.lasso.results$w)))
  stopifnot(all(colnames(all.features) == names(clustering.results$groups)))

	peak.scores <- determine.peak.scores(all.features, all.labels,
                                       group.lasso.results$w,
	...
	for (group in test.groups) {
    message(group)
    gpeaks <- full.regions.list[which(group.members[,group] == 1)]
    if (is.null(gpeaks$name)) {
      gpeaks$name <- sprintf("%s_%d", group, seq(length(gpeaks)))
    }
    SeqGL:::granges.to.bed(gpeaks, sprintf("%s/%s_members.bed", group.dir, group))
  }
